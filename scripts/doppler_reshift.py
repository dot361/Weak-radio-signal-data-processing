...
if any(re.findall(r'panstarrs|atlas|swan', logs[0], re.IGNORECASE)):
...
print(velocities_horizons)
for p in range(1, len(q)+1):
    print(q[p-1])
    if("dat.dat" not in q[p-1]):    
    ...
    for measure in range(0,p):
        ...
        print("start - ", freq)
        reshift = float(log["header"]["f_obs,LO,IF"][0]) - float(log["header"]["v_obs,v_rad,f_lab"][2])
        print("reshift - ", reshift)
        freq = freq - reshift
        print(freq)
        if("dat.dat" not in q[0]):
        ...
        velocitys_1_avg += freq_1
        velocitys_2_avg += freq_2
...
plt.subplot(1,2,1)
plt.axvline(x=1665.402, color='r')

plt.yticks(np.arange(-10, 10, step=0.05))
#plt.xticks(np.arange(-300, 300, step=20))
plt.xlabel("Frequency (MHz)")
plt.ylabel("Flux density (Jy)")
plt.plot(velocitys_1_avg, y_1_left_avg)
...
plt.suptitle("1.6673590")
plt.subplot(1,2,1)
plt.axvline(x=1667.359, color='r')
plt.yticks(np.arange(-10, 10, step=0.05))
#plt.xticks(np.arange(-300, 300, step=20))
plt.xlabel("Frequency (MHz)")
plt.ylabel("Flux density (Jy)")
...
avgarray = {}
avgarray["1665x"] = list(velocitys_1_avg)
avgarray["1665l"] = list(y_1_left_avg)
avgarray["1665r"]= list(y_1_right_avg)

avgarray["1667x"] = list(velocitys_2_avg)
avgarray["1667l"] = list(y_2_left_avg)
avgarray["1667r"] = list(y_2_right_avg)

#save to json file
with open('/home/gj/Desktop/test/work_codes/results/' + name +'reshift.json', 'w') as outfile:
